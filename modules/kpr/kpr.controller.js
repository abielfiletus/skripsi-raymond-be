  ...
  async recommendation(req, res, next) {
    try {
      ...
      for (let i = 0; i < tenor * 12; i++) {
        const pmt = KprFunc.pmt(interest / 12, i + 1, totalPinjaman, 0);
        const ipmtForPpmt = KprFunc.ipmt(totalPinjaman, pmt, i + 1);
        const ppmt = KprFunc.ppmt(pmt, ipmtForPpmt);
        const ipmt = KprFunc.ipmt(totalPinjaman, pmt, interest * 12, i + 1);
        pokok += ppmt;
        bunga += ipmt;
        cicilan = ppmt + ipmt;
        totalBayar += cicilan;
